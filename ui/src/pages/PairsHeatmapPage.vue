...
const defaultOptions = computed((): PredefinedGraphOption<"heatmap">[] | undefined => {
  ...
  const cdr3AlphaIndex = pcols.findIndex(
    (p) =>
      p.spec.domain?.["pl7.app/vdj/feature"] === "CDR3" &&
      p.spec.axesSpec[0].domain?.["pl7.app/vdj/chain"] === "TCRAlpha",
  );
  const cdr3BetaIndex = pcols.findIndex(
    (p) =>
      p.spec.domain?.["pl7.app/vdj/feature"] === "CDR3" &&
...
const key = computed(() =>
  [(app.model.args.numerators ?? []).join(","), (app.model.args.denominators ?? []).join(",")].join(
    "|",
  ),
);
</script>
